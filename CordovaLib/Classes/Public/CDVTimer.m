...
- (void)log
{
    DLog(@"[CDVTimer][%@] %fms", self.name, [self.ended timeIntervalSinceDate:self.started] * 1000.0);
}
...
- (void)add:(NSString*)name
{
    if ([self.items objectForKey:[name lowercaseString]] == nil) {
        ...
        DLog(@"Timer called '%@' already exists.", name);
    }
}
...
- (void)remove:(NSString*)name
{
    ...
    if (item != nil) {
        ...
        DLog(@"Timer called '%@' does not exist.", name);
    }
}
