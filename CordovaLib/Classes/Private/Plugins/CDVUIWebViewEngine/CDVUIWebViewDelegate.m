        ...
        if (isTopLevelNavigation) {
            ...
            switch (_state) {
                case STATE_WAITING_FOR_LOAD_FINISH:
                    ...
                    if (_loadCount != 1) {
                        DLog(@"CDVWebViewDelegate: Detected redirect when loadCount=%ld", (long)_loadCount);
                    }
                    break;
...
- (void)webViewDidStartLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            if (_loadCount != 0) {
                DLog(@"CDVWebViewDelegate: Unexpected loadCount in didStart. count=%ld", (long)_loadCount);
            }
            fireCallback = YES;
            _state = STATE_WAITING_FOR_LOAD_FINISH;
            _loadCount = 1;
        ...
        default:
            DLog(@"CDVWebViewDelegate: Unexpected didStart with state=%ld loadCount=%ld", (long)_state, (long)_loadCount);
    }
...
- (void)webViewDidFinishLoad:(UIWebView*)webView
{
    ...
    switch (_state) {
        ...
        case STATE_WAITING_FOR_LOAD_START:
            DLog(@"CDVWebViewDelegate: Unexpected didFinish while waiting for load start.");
            break;
