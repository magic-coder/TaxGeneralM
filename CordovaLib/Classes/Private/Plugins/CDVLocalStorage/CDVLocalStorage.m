...
+ (void)__verifyAndFixDatabaseLocations
{
    ...
    if (modified) {
        ...
        DLog(@"Fix applied for database locations?: %@", ok ? @"YES" : @"NO");
    }
}
...
+ (void)__restoreLegacyDatabaseLocationsWithBackupType:(NSString*)backupType
{
    ...
    for (CDVBackupInfo* info in backupInfo) {
        if ([manager fileExistsAtPath:info.backup]) {
            if ([info shouldRestore]) {
                DLog(@"Restoring old webstorage backup. From: '%@' To: '%@'.", info.backup, info.original);
                [self copyFrom:info.backup to:info.original error:nil];
            }
            DLog(@"Removing old webstorage backup: '%@'.", info.backup);
...
- (void)onResignActive
{
    ...
    if (exitsOnSuspend) {
        ...
        backgroundTaskID = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
                ...
                DLog(@"Background task to backup WebSQL/LocalStorage expired.");
            }];
