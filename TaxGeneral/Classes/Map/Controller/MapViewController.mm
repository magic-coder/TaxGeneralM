...
- (void)onGetDrivingRouteResult:(BMKRouteSearch*)searcher result:(BMKDrivingRouteResult*)result errorCode:(BMKSearchErrorCode)error{
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        [YZAlertView showAlertWith:self title:@"没有对应驾乘线路" message:@"建议使用其他方式前往" callbackBlock:^(NSInteger btnIndex) {
...
- (void)onGetMassTransitRouteResult:(BMKRouteSearch*)searcher result:(BMKMassTransitRouteResult*)result errorCode:(BMKSearchErrorCode)error{
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        [YZAlertView showAlertWith:self title:@"没有对应公交线路" message:@"建议使用其他方式前往" callbackBlock:^(NSInteger btnIndex) {
        } cancelButtonTitle:@"确定" destructiveButtonTitle:nil otherButtonTitles: nil];
    }
}
#pragma mark 步行路线
- (void)onGetWalkingRouteResult:(BMKRouteSearch*)searcher result:(BMKWalkingRouteResult*)result errorCode:(BMKSearchErrorCode)error{
    ...
    if (error == BMK_SEARCH_NO_ERROR) {
        ...
        [YZAlertView showAlertWith:self title:@"没有对应步行线路" message:@"建议使用其他方式前往" callbackBlock:^(NSInteger btnIndex) {
        } cancelButtonTitle:@"确定" destructiveButtonTitle:nil otherButtonTitles: nil];
    }
}
...
- (void)moreAction:(UIButton *)sender{
    ...
    items = [@[[YZMenuItem menuItem:@"驾乘路线"
                                image:nil
                                  tag:100
                             userInfo:@{@"title":@"Menu"}],
                [YZMenuItem menuItem:@"公交路线"
                                image:nil
                                  tag:101
                             userInfo:@{@"title":@"Menu"}],
                [YZMenuItem menuItem:@"步行路线"
                                image:nil
                                  tag:102
                             userInfo:@{@"title":@"Menu"}]
                ] mutableCopy];
    ...
    UIButton *btn = sender;
    [YZMenu showMenuInView:self.view fromRect:btn.frame menuItems:items selected:^(NSInteger index, YZMenuItem *item) {
        if(item.tag == 100){
