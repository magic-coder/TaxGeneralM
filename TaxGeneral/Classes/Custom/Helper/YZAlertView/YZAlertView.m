/************************************************************
 Class    : YZAlertView.m
 Describe : 自己封装的alert弹出提示框提供便捷的弹出框
 Company  : Prient
 Author   : Yanzheng
 Date     : 2016-07-26
 Version  : 1.0
...
#pragma mark - UIAlertView类别扩充
//类别，扩充属性
@interface UIAlertView (YZAlertView)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIAlertView (YZAlertView)
...
//类别，扩充属性
@interface UIActionSheet (YZAlertView)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIActionSheet (YZAlertView)
- (void)setClickBlock:(CallBackBlock)block
{
    objc_setAssociatedObject(self, @selector(clickBlock), block, OBJC_ASSOCIATION_COPY);
}
...
#pragma mark - 接口
@interface YZAlertView () <UIAlertViewDelegate, UIActionSheetDelegate>
...
+ (void)showTipAlertViewWith:(UIViewController *)viewController
                 ...
                 buttonStyle:(YZAlertActionStyle)btnStyle
{
    if (iOS_Version >= 8.0)
    {
        ...
        if (btnTitle) {
            ...
            switch (btnStyle) {
                case YZAlertActionStyleDefault:
           ...
           otherButtonStyleArray:(NSArray *)otherBtnStyleArray
{
    ...
    {
        //关联代理有警告，但是不能用对象否则代理无效 Yan -> 去除代理警告
        UIActionSheet * actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:(id<UIActionSheetDelegate>)self cancelButtonTitle:cancelBtnTitle destructiveButtonTitle:destructiveBtnTitle otherButtonTitles:nil];
        actionSheet.clickBlock = block;
